...
class WaitlistPersistenceAdapterTest {
    ...
    @Test
    @DisplayName("save con ID inexistente lanza excepción de registro no encontrado")
    void saveExistenteNoEncontrado() {
        Paciente paciente = Paciente.builder().id(pacienteEntity.getId())
                .rut("12345678-9").nombre("María").apellido("González").build();
        ...
        assertThrows(WaitlistItemNotFoundException.class, () -> adapter.save(item));
    }
